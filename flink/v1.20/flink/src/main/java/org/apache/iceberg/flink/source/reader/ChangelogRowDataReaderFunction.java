...
import org.apache.iceberg.flink.source.ChangelogScanTaskReader;
  ...
  public DataIterator<RowData> createDataIterator(IcebergSourceSplit split) {
    if (split.isChangelogSplit()) {
      return new ChangelogDataIterator(
          new ChangelogScanTaskReader(tableSchema, readSchema, nameMapping, caseSensitive),
          split.changelogTasks(),
          io,
          encryption);
    }

    // Fallback for regular splits
    return new DataIterator<>(
        new RowDataFileScanTaskReader(tableSchema, readSchema, nameMapping, caseSensitive, null),
        split.task(),
        io,
        encryption);
  }

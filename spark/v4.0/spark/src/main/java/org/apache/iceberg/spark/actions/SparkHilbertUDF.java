...
import org.apache.iceberg.util.HilbertCurve;
...
/**
 * Spark UDF helper for Hilbert curve-based liquid clustering.
 *
 * <p>Mirrors {@link SparkZOrderUDF} but replaces bit-interleaving with a proper N-dimensional
 * Hilbert index ({@link HilbertCurve#toIndex}). All type-conversion UDFs ({@link
 * ZOrderByteUtils}) are reused unchanged.
 */
class SparkHilbertUDF implements Serializable {
  ...
  byte[] hilbertIndex(Seq<byte[]> scalaBinary) {
    byte[][] columnsBinary = JavaConverters.seqAsJavaList(scalaBinary).toArray(inputHolder.get());
    return HilbertCurve.toIndex(columnsBinary, totalOutputBytes);
  }
  ...
  private final UserDefinedFunction hilbertUDF =
      functions
          .udf((Seq<byte[]> arrayBinary) -> hilbertIndex(arrayBinary), DataTypes.BinaryType)
          .withName("HILBERT_INDEX");

  Column hilbertIndexCol(Column arrayBinary) {
